		...
		function _buildBody(canlederBox){
			...
			for(var i = 0; i < 12; i++){
				...
				var $li = $("<li/>").append($inner).click(function(){
					...
					if(wwjP==1){//选择范围当前日期及前
						
						if(wwjp_month!=''){
							boxMonth=wwjp_month;
						}
						if ((year+ "-" +month) < boxMonth) {
							canlederBox.data("box").val(year + "-" + month);
						}else{
							canlederBox.data("box").val(boxMonth);//year + "-0" + boxMonth
						};
					}else if(wwjP==2){//选择范围当前日期及后
						if(wwjp_month!=''){
							boxMonth=wwjp_month;
						}
						if ((year+ "-" +month) > boxMonth) {
